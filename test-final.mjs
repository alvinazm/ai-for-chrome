...
for (const tc of testCases) {
  ...
  try {
    ...
    console.log('After removeThinkTags:', cleaned.substring(0, 80) + (cleaned.length > 80 ? '...' : ''));
    
    const parsed = extractJsonFromModelOutput(cleaned);
    console.log('Parsed:', JSON.stringify(parsed).substring(0, 80));
    console.log('SUCCESS!\n');
  } catch (e) {
